...
-- 4a) If product price is to be increased by 10%
SELECT 
product_name, 
unit_price, 
...
-- 4b) To determine products that have high quantity still available/in stock
SELECT *
FROM products
WHERE quantity_in_stock > 50
ORDER BY quantity_in_stock;

-- 4c) top selling products limit by 5
SELECT 
p.product_name,
COUNT(product_name)
FROM products p
...
-- 4d) TO identify products by name and price at the time of oder 
SELECT 
oi.order_id, 
oi.product_id, 
p.product_name, 
...
-- 5c) To identify customers, orders and status of their order
SELECT o.order_id,
o.order_date,
c.first_name,
c.last_name,
...
-- 5e) To identify most used shipper/logistics company
SELECT 
s.shipper_name,
COUNT(o.shipper_id)
FROM shippers s
